...
class SalObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, depth_root, trainsize):
        ...
        self.images = [os.path.join(image_root, f) for f in os.listdir(image_root) if f.endswith(('.jpg', '.png', '.bmp'))]
        self.gts = [os.path.join(gt_root, f) for f in os.listdir(gt_root) if f.endswith(('.jpg', '.png', '.bmp'))]
        self.depths = [os.path.join(depth_root, f) for f in os.listdir(depth_root) if f.endswith(('.bmp', '.png', '.jpg'))]

        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
        self.depths = sorted(self.depths)

        print(f"Before filtering: {len(self.images)} images")
        # self.filter_files()  # 你可以根据需要决定要不要开启过滤
        print(f"After filtering: {len(self.images)} images")
    ...
    def resize(self, img, gt, depth):
        w = max(img.size[0], gt.size[0], depth.size[0], self.trainsize)
        h = max(img.size[1], gt.size[1], depth.size[1], self.trainsize)
