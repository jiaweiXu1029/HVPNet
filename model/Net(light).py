...
class Net(nn.Module):
    def __init__(self, pretrained=True,channel=32):
        super().__init__()
        self.rgb_backbone = mobilenet_v2(pretrained)
        self.d_backbone = mobilenet_v2(pretrained)
        # Fuse
        self.gfm2 = GFM(inc=320, expend_ratio=2)
        self.SCA3 = SCA(96)
        self.SCA2 = SCA(32)
        self.SCA1 = SCA(24)
        self.EDS = EDS(16)
        # Pred
        self.mcm2 = MCM(inc=32, outc=24)
        self.mcm1 = MCM(inc=24, outc=16)
        self.rfb5_1 = RFB(320, channel)
        self.rfb4_1 = RFB(96, channel)
        self.rfb3_1 = RFB(32, channel)
        ...
        self.predtrans4 = nn.Sequential(
            nn.Conv2d(in_channels=320, out_channels=320, kernel_size=3, stride=1, padding=1, groups=320),
            nn.BatchNorm2d(320),
            nn.GELU(),
            nn.Conv2d(in_channels=320, out_channels=1, kernel_size=1, stride=1)
        )
    ...
    def forward(self, x_rgb, x_d):
        # rgb
        rgb_1, rgb_2, rgb_3, rgb_4,rgb_5 = self.rgb_backbone(x_rgb)
        # d
        d_1, d_2, d_3, d_4,d_5 = self.d_backbone(x_d)
        # Fuse
        fuse_5 = self.gfm2(rgb_5, d_5)
        pred_4 = F.interpolate(self.predtrans4(fuse_5), TRAIN_SIZE, mode="bilinear", align_corners=True)
        fuse_4 = self.SCA3(rgb_4, d_4)
        fuse_3 = self.SCA2(rgb_3, d_3)
        x3=fuse_3
        fuse_2 = self.SCA1(rgb_2, d_2)
        fuse_1 = self.EDS(rgb_1, d_1)  # [B, 64, 96, 96]

        fuse_5=self.rfb5_1(fuse_5)
        fuse_4 = self.rfb4_1(fuse_4)
        fuse_3 = self.rfb3_1(fuse_3)
        attention_map = self.agg1(fuse_5, fuse_4, fuse_3)
        fuse_3 = self.HA(attention_map.sigmoid(), x3)
        pred_3 = F.interpolate(self.predtrans3(fuse_3), TRAIN_SIZE, mode="bilinear", align_corners=True)
        pred_2, xf_2 = self.mcm2(fuse_2, fuse_3)
