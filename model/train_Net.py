...
parser.add_argument('--rgb_root', type=str, default='RGBD_train/RGB/',
                    help='the training rgb images root')  # train_dut
parser.add_argument('--depth_root', type=str, default='RGBD_train/depth/',
                    help='the training depth images root')
parser.add_argument('--gt_root', type=str, default='RGBD_train/GT/',
                    help='the training gt images root')
parser.add_argument('--val_rgb', type=str, default="RGBD_train/val/RGB/",
                    help='validate rgb path')
parser.add_argument('--val_depth', type=str, default="RGBD_train/val/depth/",
                    help='validate depth path')
parser.add_argument('--val_gt', type=str, default="RGBD_train/val/GT/",
...
parser.add_argument('--save_path', type=str, default="ckps/Net/", help='checkpoint path')
...
if os.path.exists("ckps/smt/smt_tiny.pth"):
    #checkpoint = torch.load("ckps/smt/mobilenet_v2-b0353104.pth")
    checkpoint = torch.load("ckps/smt/smt_tiny.pth")
    print(checkpoint.keys())  # 打印所有键，确认是否有 'model' 键
    if 'model' in checkpoint:
        model.rgb_backbone.load_state_dict(checkpoint['model'])
        print(f"loaded imagenet pretrained SMT from ckps/smt/smt_base.pth")
    else:
        print("Error: 'model' key not found in checkpoint")
else:
    raise FileNotFoundError("please put smt_tiny.pth under ckps/smt/ folder")

# 如果继续训练
if opt.continue_train:
    if os.path.exists(opt.continue_train_path):
        model.load_state_dict(torch.load(opt.continue_train_path))
        print(f"continue training from {opt.continue_train_path}")
    else:
        raise FileNotFoundError(f"Checkpoint file not found at {opt.continue_train_path}")
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        loss4 = weight4*(CE(pred_4, gts) + iou_loss(pred_4, gts))
